...
def kpi_card(title, value, unit=""):
    """
    Mostra una KPI card uniforme, con valore sempre contenuto
    nello spazio disponibile.
    """

    value_fmt = format_value(value, unit)

    # Dimensione adattiva in base alla lunghezza del valore
    lunghezza = len(str(value_fmt))

    if lunghezza >= 18:
        dimensione_valore = "25px"
    elif lunghezza >= 14:
        dimensione_valore = "28px"
    elif lunghezza >= 10:
        dimensione_valore = "31px"
    else:
        dimensione_valore = "34px"

    st.markdown(
        f"""
        <div class="sbl-kpi-card">
            <div class="sbl-kpi-title">
                {title}
            </div>

            <div
                class="sbl-kpi-value"
                style="font-size:{dimensione_valore};"
            >
                {value_fmt}
            </div>
        </div>
