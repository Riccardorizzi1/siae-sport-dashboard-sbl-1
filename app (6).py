...
# Palette ad alto contrasto, costruita sui colori aziendali:
# blu / arancione / verde acqua / rosa-rosso / viola
COLORI_AZIENDALI = [
    "#00649C",  # blu scuro
    "#E8901C",  # arancione
    "#10E8A4",  # verde acqua
    "#F55D78",  # rosa/rosso
    "#7161FF",  # viola
    "#E8AA05",  # giallo/arancio
    "#00A2D2",  # azzurro
    "#9C030C",  # rosso scuro
    "#64E8CB",  # turchese chiaro
    "#495B69",  # grigio blu
]

COLORI_MACROAREE = {
    "Nord-Ovest": "#00649C",  # blu scuro
    "Nord-Est": "#E8901C",    # arancione
    "Centro": "#10E8A4",      # verde acqua
    "Sud": "#F55D78",         # rosa/rosso
    "Isole": "#7161FF",       # viola
}

COLORI_CATEGORIE = {
    "Attività sportiva": "#00649C",
    "Sport calcio": "#E8901C",
    "Sport di squadra non calcio": "#10E8A4",
    "Sport individuali": "#7161FF",
    "Altri sport": "#F55D78",
}
...
def applica_layout_plotly(fig, titolo=None):
    fig.update_layout(
        title=titolo if titolo else fig.layout.title.text,
        template="plotly_white",
        colorway=COLORI_AZIENDALI,
        font=dict(
            family="Arial",
            size=13,
            color="#082B45"
    ...
    # Migliora leggibilità delle linee: colore + spessore + tratteggio + marker
    for trace in fig.data:
        nome = str(trace.name)

        if nome in COLORI_MACROAREE:
            trace.update(
                line=dict(
                    color=COLORI_MACROAREE[nome],
                    width=3.5,
                    dash=DASH_MACROAREE.get(nome, "solid")
                ),
                marker=dict(
                    size=7,
                    color=COLORI_MACROAREE[nome]
        ...
        elif nome in COLORI_CATEGORIE:
            trace.update(
                marker=dict(
                    color=COLORI_CATEGORIE[nome]
                )
            )
...
def _sbl_is_year_column(col):
    """
    Riconosce colonne che rappresentano anni o periodi temporali.
    """
    col_lower = str(col).lower()
    return (
        col_lower in ["anno", "anni", "year", "periodo"]
        or "anno" in col_lower
        or "year" in col_lower
        or "periodo" in col_lower
    )
...
def _sbl_clean_year_value(value):
    """
    Corregge gli anni visualizzati male:
    - 2021.0 -> 2021
    - 2.021 -> 2021
    - 2,021 -> 2021
    - 2004 -> 2004
...
def _sbl_fix_year_columns_for_display(data):
    """
    Applica il fix solo alle colonne anno/year/periodo delle tabelle.
    Non tocca gli altri numeri.
    """
    try:
        ...
        for col in df.columns:
            if _sbl_is_year_column(col):
                df[col] = df[col].apply(_sbl_clean_year_value)
...
def _sbl_fix_plotly_year_axis(fig):
    """
    Evita che gli anni sull'asse X appaiano come 2.021 o 2,021.
    """
    try:
        anni_trovati = []
        ...
        for trace in fig.data:
            if hasattr(trace, "x") and trace.x is not None:
                for v in list(trace.x):
                    anno = _sbl_clean_year_value(v)
                    if isinstance(anno, str) and re.fullmatch(r"(19|20)\d{2}", anno):
                        anni_trovati.append((v, anno))

        if len(anni_trovati) > 0:
            visti = set()
            tickvals = []
            ticktext = []

            for originale, anno in anni_trovati:
                if anno not in visti:
                    visti.add(anno)
                    tickvals.append(originale)
                    ticktext.append(anno)
...
# Patch finale Streamlit: viene applicata dopo le altre patch già presenti
if "_SBL_FIX_ANNI_DASHBOARD_ACTIVE" not in globals():
    _SBL_FIX_ANNI_DASHBOARD_ACTIVE = True

    _sbl_original_dataframe_fix_anni = st.dataframe
    _sbl_original_table_fix_anni = st.table
    _sbl_original_plotly_fix_anni = st.plotly_chart
    _sbl_original_selectbox_fix_anni = st.selectbox

    def _sbl_dataframe_fix_anni(data=None, *args, **kwargs):
        return _sbl_original_dataframe_fix_anni(
            _sbl_fix_year_columns_for_display(data),
            *args,
            **kwargs
        )

    def _sbl_table_fix_anni(data=None, *args, **kwargs):
        return _sbl_original_table_fix_anni(
            _sbl_fix_year_columns_for_display(data),
            *args,
            **kwargs
        )

    def _sbl_plotly_chart_fix_anni(fig_or_data, *args, **kwargs):
        fig_or_data = _sbl_fix_plotly_year_axis(fig_or_data)
        return _sbl_original_plotly_fix_anni(fig_or_data, *args, **kwargs)

    def _sbl_selectbox_fix_anni(label, options, *args, **kwargs):
        label_lower = str(label).lower()

        if (
            "anno" in label_lower
            or "year" in label_lower
            or "periodo" in label_lower
        ) and "format_func" not in kwargs:
            kwargs["format_func"] = lambda x: str(_sbl_clean_year_value(x))

        return _sbl_original_selectbox_fix_anni(label, options, *args, **kwargs)

    st.dataframe = _sbl_dataframe_fix_anni
    st.table = _sbl_table_fix_anni
    st.plotly_chart = _sbl_plotly_chart_fix_anni
    st.selectbox = _sbl_selectbox_fix_anni

    try:
        _sbl_original_data_editor_fix_anni = st.data_editor

        def _sbl_data_editor_fix_anni(data=None, *args, **kwargs):
            return _sbl_original_data_editor_fix_anni(
                _sbl_fix_year_columns_for_display(data),
                *args,
                **kwargs
            )

        st.data_editor = _sbl_data_editor_fix_anni

    except Exception:
        pass

# =========================
# FINE FIX ANNI DASHBOARD
# =========================
...
if pagina == "Panoramica nazionale":
    ...
    Il periodo analizzato include anche la forte discontinuità legata alla pandemia, particolarmente
    evidente tra il **2020 e il 2021**. Questo aspetto richiede una lettura separata rispetto alle
    dinamiche osservate negli anni precedenti.

    ---

    ### Indicatori considerati

    La dashboard utilizza un insieme di indicatori pensati per descrivere le principali dimensioni del fenomeno:

    - **numero di spettacoli sportivi**, come misura dell’offerta di eventi;
    - **persone partecipanti**, come indicatore della dimensione del pubblico;
    - **persone medie per spettacolo**, per valutare l’intensità media della partecipazione;
    - **quota regionale sul totale nazionale**, per confrontare il peso relativo dei territori;
    - **botteghino**, dove disponibile, come misura della dimensione economica degli eventi;
    - **botteghino medio per spettacolo**, come indicatore della capacità media di generare ricavi;
    - **indicatori di resilienza**, utili a leggere il recupero dei territori dopo la contrazione del 2020.

    Per il **2021** è disponibile un livello di dettaglio maggiore sulle categorie sportive, che consente
    di distinguere tra attività sportiva complessiva, calcio, altri sport di squadra, sport individuali
    e altre discipline.
    ...
    La dashboard è organizzata per accompagnare l’utente da una visione generale a una lettura più dettagliata.

    La **panoramica nazionale** sintetizza i principali indicatori disponibili.
    I **trend temporali** mostrano l’evoluzione degli eventi sportivi nel periodo 2004-2021.
    I **ranking regionali** permettono di confrontare i territori in base all’indicatore selezionato.
    La **mappa Italia** offre una lettura immediata della distribuzione regionale dei valori.
    Il **focus categorie** approfondisce la composizione degli eventi sportivi per tipologia.
    La **scheda regionale** consente di analizzare il profilo di una singola regione.
    Le **tabelle dati** permettono infine di consultare direttamente le principali informazioni elaborate.

    ---

    ### Criteri di interpretazione

    L’analisi ha finalità **descrittiva e comparativa**. I risultati consentono di individuare tendenze,
    differenze territoriali e dinamiche di partecipazione, ma non devono essere interpretati come evidenza
    di relazioni causali dirette.

    Alcuni indicatori, in particolare quelli economici e quelli legati al dettaglio per categoria sportiva,
    non sono disponibili con la stessa granularità per tutti gli anni. Per questo motivo, alcune visualizzazioni
    mostrano esclusivamente le informazioni effettivamente presenti nel dataset.

    I valori aggregati a livello nazionale o macroterritoriale possono inoltre nascondere differenze significative
    tra singole regioni. Per una lettura completa è quindi opportuno combinare la visione generale con il dettaglio territoriale.

    La discontinuità del periodo pandemico rappresenta un elemento centrale dell’analisi: il confronto tra il 2020,
    il 2021 e gli anni precedenti deve essere letto tenendo conto del carattere eccezionale di quella fase.
    """)
...
# =====================================================
    ...
    macro["territorio"] = pd.Categorical(
        macro["territorio"],
        categories=ordine_macroaree,
        ordered=True
    )
    ...
    fig = px.line(
        ...
        labels={
            "anno": "Anno",
            indicatore_macroarea: indicatore_macroarea,
            "territorio": "Macroarea"
        },
        color_discrete_map=COLORI_MACROAREE
    )
